...
两种渲染共享选择顺序：所有浮点属性有限 → 可选 opacity 阈值 → 可选 source-coordinate bounds → 可选轨迹局部裁切 → 可选均匀索引采样。非有限属性属于无效数据检查，不属于空间去噪。未启用轨迹时默认不设 opacity 阈值、不裁剪、不采样。轨迹模式额外记录 `after_route_crop`；导出全量裁切结果，预览采样只影响渲染。
...
相机为正交投影。azimuth 单位度；elevation 为 (0,90]；内部使用源坐标相机变换，Gaussian 的四元数不需要因显示旋转重新写入。框选以点中心范围为依据；轨迹模式额外包含完整路线和视锥角点。`framing` 默认0.78、范围 `(0,0.90]`，调小可拉远。极大的 Gaussian 椭球仍可能延伸到图片外。
...
`render --trajectory` 接受已对齐的 XYZ CSV，`--route-config` 接受独立扁平 JSON。仅移除相邻重复位置，不重新规划路线。视锥使用三维切线，是方向示意。输出额外包含属性保持的 `.crop.ply` 与 `.trajectory.csv`，也受禁止覆盖保护；原始数据不被修改。坐标约定、参数和裁切公式见 [轨迹绘制方法](trajectory_rendering.md)。
